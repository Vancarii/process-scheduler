...
void send_command(char* pid_c, char* msg) {
    ...
    printf("Process with pid [%d] now gets CPU.\n", current_process->pid);
...
void recieve_command() {
    ...
    if (current_process->proc_messages[0] != '\0') {
        printf("Successfully received from PID [%d] with message: %s\n",current_process->sender_pid, current_process->proc_messages);
        printf("Process continues execution.\n");
...
void reply_command(char* pid_c, char* msg) {
    ...
    if (target_queue == NULL){
        ...
        strncpy(found_process->proc_messages, msg, PROC_MESSAGES_SIZE - 1);
        found_process->proc_messages[PROC_MESSAGES_SIZE - 1] = '\0';
        printf("Reply successfully sent to init process with message: %s\n", found_process->proc_messages);
        found_process->sender_pid = current_process->pid;
        printf("Previous process pid [%d] continues execution.\n", current_process->pid);
        ...
        if ((found_process = List_remove(waiting_reply_queue)) == NULL) {
            ...
            strncpy(found_process->proc_messages, msg, PROC_MESSAGES_SIZE - 1);
            found_process->proc_messages[PROC_MESSAGES_SIZE - 1] = '\0';
            printf("Reply successfully sent to sender process with PID [%d] with message: %s\n", pid, found_process->proc_messages);
            found_process->sender_pid = current_process->pid;
